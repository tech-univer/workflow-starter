...
public class ProcessCoinFlips implements JavaDelegate {
    ...
    @Autowired
    public ProcessCoinFlips(RuntimeService runtimeService, TaskService taskService, CoinFlipRepository repository) {
    ...
    public void execute(DelegateExecution delegateExecution) throws Exception {
        ...
        variables.forEach((k, v) -> {
            if (k.startsWith("flipResult")) {
                flips.add(new Flip(delegateExecution.getProcessBusinessKey(), UUID.randomUUID().toString(), Outcome.valueOf(v.toString()), currency, denomination));
            }
        });
